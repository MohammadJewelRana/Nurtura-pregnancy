    ...
    <div className="space-y-6">
      ...
      <div className="p-6 sm:p-8 bg-white dark:bg-charcoal-900 rounded-3xl border border-rose-100 dark:border-charcoal-800 shadow-soft">
        <div className="flex items-center space-x-3 mb-4">
          <div className="p-3 rounded-2xl bg-amber-50 dark:bg-amber-950/40 text-amber-500">
        ...
        <form onSubmit={handleSaveMood} className="space-y-5">
                ...
                return (
                  <button
                    ...
                    className={`p-3 rounded-2xl border text-center transition-all flex flex-col items-center justify-center ${
                      isSelected
                        ? 'bg-amber-50 dark:bg-amber-950/40 border-amber-300 dark:border-amber-700 shadow-sm scale-105'
                        : 'bg-white dark:bg-charcoal-800 border-rose-100 dark:border-charcoal-700 hover:bg-rose-50/50'
                    }`}
                  >
          ...
          <div>
            <textarea
              ...
              className="w-full px-4 py-2.5 rounded-2xl border border-rose-200 dark:border-charcoal-700 bg-white dark:bg-charcoal-800 text-xs sm:text-sm text-charcoal-800 dark:text-white focus:outline-none focus:ring-2 focus:ring-amber-400"
            />
          </div>
          ...
          <div className="flex items-center justify-between pt-1">
            ...
            <button
              type="submit"
              className="px-5 py-2.5 rounded-2xl bg-amber-500 hover:bg-amber-600 text-white text-xs font-semibold shadow-soft transition flex items-center space-x-1.5"
            >
            ...
            {moods.map((m) => (
              <div key={m.id} className="py-3 flex items-start justify-between text-xs gap-3">
                <div className="flex items-start space-x-3">
                  ...
                  <div>
                    <span className="font-semibold text-charcoal-900 dark:text-white block">
                      {formatDate(m.date)}
                    </span>
                    {m.symptoms && m.symptoms.length > 0 && (
                      <div className="flex flex-wrap gap-1 mt-1">
                        {m.symptoms.map((s) => (
                          <span
                            key={s}
                            className="px-2 py-0.5 rounded-md bg-rose-50 dark:bg-charcoal-800 text-[10px] text-rose-600 dark:text-rose-400 font-medium"
                          >
                            {s}
                          </span>
                        ))}
