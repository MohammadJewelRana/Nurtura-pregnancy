...
export function PregnancyCalendar() {
  ...
  if (!calculation.isValid) {
    return (
      <div className="p-8 text-center bg-white dark:bg-charcoal-900 rounded-3xl border border-rose-100 dark:border-charcoal-800 shadow-soft">
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
      ...
      <div className="lg:col-span-2 p-5 sm:p-7 bg-white dark:bg-charcoal-900 rounded-3xl border border-rose-100 dark:border-charcoal-800 shadow-soft">
        {/* Month Header & Controls */}
        <div className="flex items-center justify-between mb-6">
          <div className="flex items-center space-x-2">
            <button
              onClick={prevMonth}
              className="p-2 rounded-xl border border-rose-200 dark:border-charcoal-700 bg-white dark:bg-charcoal-800 text-charcoal-700 dark:text-charcoal-300 hover:bg-rose-50 transition"
              aria-label="Previous month"
            >
              <ChevronLeft className="w-4 h-4" />
            </button>
            <h3 className="text-base sm:text-lg font-bold text-charcoal-900 dark:text-white px-2">
            ...
            return (
              <button
                ...
                className={`relative min-h-[52px] sm:min-h-[64px] p-1.5 rounded-2xl flex flex-col justify-between items-center transition text-xs ${
                  isSelected
                    ? 'bg-rose-500 text-white font-bold shadow-soft'
                    : isToday
                    ...
                    : isCurrentM
                    ? 'bg-cream-50/50 dark:bg-charcoal-800/30 text-charcoal-800 dark:text-charcoal-200 hover:bg-rose-50/70 dark:hover:bg-charcoal-800'
                  ...
                  <span
                    className={`text-[9px] px-1 py-0.5 rounded-md truncate max-w-full ${
                      isSelected
                        ...
                        : isEdd
                        ? 'bg-amber-400 text-charcoal-900 font-extrabold'
                        : 'text-rose-600 dark:text-rose-400'
                    }`}
                  >
                    {isEdd ? (language === 'bn' ? 'EDD' : 'EDD') : `W${formatNumber(dayStats.week)}`}
                  </span>
                )}
              </button>
            );
      ...
      <div className="p-6 bg-white dark:bg-charcoal-900 rounded-3xl border border-rose-100 dark:border-charcoal-800 shadow-soft flex flex-col justify-between space-y-5">
        <div>
          ...
          {selectedDateStats ? (
            <div className="mt-4 space-y-4 text-xs">
              <div className="p-4 rounded-2xl bg-rose-50/50 dark:bg-charcoal-800/50 border border-rose-100 dark:border-charcoal-700">
                ...
                <span className="block mt-1 text-[11px] text-charcoal-500 dark:text-charcoal-400 font-medium">
              ...
              {weekData && (
                <div className="p-4 rounded-2xl bg-cream-50 dark:bg-charcoal-800/40 border border-cream-200 dark:border-charcoal-700">
                  <span className="text-charcoal-400 block mb-1 font-semibold flex items-center gap-1">
                  ...
                  <p className="mt-2 text-[11px] text-rose-600 dark:text-rose-400 font-bold">
            ...
            <div className="mt-4 p-4 rounded-2xl bg-charcoal-50 dark:bg-charcoal-800/40 text-xs text-charcoal-500">
              {language === 'bn'
        ...
        <div className="pt-4 border-t border-rose-100 dark:border-charcoal-800 text-xs text-charcoal-500 dark:text-charcoal-400">
          <p>
            {language === 'bn'
